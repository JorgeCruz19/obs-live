...
async function main() {
	...
	if (iOS || iPad) {
		...
		if (SafariVersion && !ChromeVersion) {
	...
	if (urlParams.has("record")) {
		if (SafariVersion) {
			if (!session.cleanOutput) {
				warnUser(
					"Your browser or device is not supported. Try Chrome if on macOS."
				);
			}
		} else {
	...
	if (
		...
		if (!session.password) {
			...
			session.password = await promptAlt(
				miscTranslations["enter-password"],
				true,
				true
			);
			if (session.password) {
	...
	if (urlParams.has("hash") || urlParams.has("crc") || urlParams.has("check")) {
		...
		if (session.password === false) {
			window.focus();
			session.password = await promptAlt(
				miscTranslations["enter-password-2"],
				true,
				true
			);
			session.password = sanitizePassword(session.password);
							...
							if (hash2.substring(0, 4) !== hash_input) {
								...
								if (!session.cleanOutput) {
									getById("request_info_prompt").innerHTML =
										miscTranslations["password-incorrect"];
	...
	if (urlParams.has("label") || urlParams.has("l")) {
		...
		if (session.label == null || session.label.length == 0) {
			...
			session.label = await promptAlt(
				miscTranslations["enter-display-name"],
				true
			);
		} else {
	...
	if (urlParams.has("remote") || urlParams.has("rem")) {
		log("remote ENABLED");
		session.remote =
			urlParams.get("remote") || urlParams.get("rem") || "nosecurity";
	...
	if (urlParams.has("margin")) {
		try {
			...
			document
				.querySelector(":root")
				.style.setProperty("--video-margin", videoMargin);
		} catch (e) {
	...
	if (urlParams.has("rounded")) {
		try {
			...
			document
				.querySelector(":root")
				.style.setProperty("--video-rounded", videoRounded);
		} catch (e) {
			errorlog("variable css failed");
		}
	}
	...
	try {
		if (urlParams.has("darkmode") || urlParams.has("nightmode")) {
			...
			darkmode = getComputedStyle(document.querySelector(":root"))
		...
		if (darkmode) {
			...
			document
				.querySelector(":root")
				.style.setProperty("--background-color", "#02050c");
		} else {
			document.body.classList.remove("darktheme");
			document
				.querySelector(":root")
				.style.setProperty("--background-color", "#141926");
		}
	} catch (e) {
		errorlog(e);
	}
	...
	if (session.videoDevice === 0) {
		...
		if (session.audioDevice === 0) {
			...
			getById("container-3").classList.add("skip-animation2");
	...
	var iceServers = [
		{
			...
			username: "jcruz@televicentro.com",
			credential: "Televicentro2022",
		},
		...
		if (urlParams.has("director") || urlParams.has("dir")) {
			...
			if (session.webcamonly == true) {
				...
				getById("head1").innerHTML =
					'<font style="color:#CCC;" data-translate="please-accept-permissions">- Please accept any camera permissions</font>';
			} else {
				getById("head1").innerHTML =
					'<br /><font style="color:#CCC" data-translate="please-select-which-to-share">- Please select which you wish to share</font>';
			}
		}
	}
	...
	if (session.roomid !== false) {
		if (!session.cleanOutput) {
			if (session.roomid === "test") {
				if (session.password === session.defaultPassword) {
					window.focus();
					var testRoomResponse = confirm(
						miscTranslations["room-test-not-good"]
					);
		...
		if (session.webcamonly == true) {
			...
			/* getById("head1").innerHTML =
				'<font style="color:#CCC" data-translate="please-select-option-to-join">Por favor carga la Cámara.</font>'; */
			getById("head1").innerHTML = "";
		}
	...
	} else if (urlParams.has("director") || urlParams.has("dir")) {
		// if I do a short form of this, it will cause duplications in the code elsewhere.
		if (directorLanding == false) {
			var director_room_input =
				urlParams.get("director") || urlParams.get("dir");
			director_room_input = sanitizeRoomName(director_room_input);
			...
			if (
				...
				if (!session.directorPassword) {
					...
					session.directorPassword = await promptAlt(
						miscTranslations["enter-director-password"],
						true
					);
				} else {
				...
				if (session.view) {
					if (document.getElementById("mainmenu")) {
						if (urlParams.has("waitimage")) {
							...
							getById("retrySpinner").title =
								miscTranslations["waiting-for-the-stream"];
						}
	...
	if (session.autohide && !session.mobile && session.scene === false) {
		// && (session.roomid!==false)){
	...
	if (isIFrame) {
		...
		window.onmessage = function (e) {
			...
			if ("sendData" in e.data) {
				...
				var ret = session.sendGenericData(
					e.data.sendData,
					UUID,
					streamID,
					type
				); // comes out the other side as: ("dataReceived", data, UUID);
				if (!ret) {
						...
						if ("streamID" in session.rpcs[i]) {
							if ("target" in e.data) {
								if (
									...
									session.rpcs[i].videoElement.volume = parseFloat(
										e.data.volume
									);
								}
							} else {
			...
			if ("sceneState" in e.data) {
				...
				if (session.obsState.visibility !== e.data.sceneState) {
						...
						for (var i = 0; i < WebMidi.inputs.length; i++) {
							...
							input.addListener("midimessage", function (e) {
								...
								for (var UUID in session.rpcs) {
									if (session.rpcs[UUID].allowMIDI) {
	...
	try {
		...
		log(e);
	} // effectiveType is not yet supported by Firefox or Safari; 2021
	...
	window.onload = function winonLoad() {
		// This just keeps people from killing the live stream accidentally. Also give me a headsup that the stream is ending
		window.addEventListener("beforeunload", function (e) {
			if (
				!session.noExitPrompt &&
				!session.cleanOutput &&
				(session.permaid !== false || session.director)
			) {
